...
class LoginController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        if (Auth::guard('krama')->attempt($credentials)) {
            ...
            return redirect('/keluarga')->with('success', 'Login Berhasil');
        } elseif (Auth::guard('prajuru')->attempt($credentials)) {
            $request->session()->regenerate();
            if (Auth::guard('prajuru')->user()->jabatan == 6) {
                return redirect()->intended('JadwalKegiatan')->with('success', 'Login Berhasil');
            }
